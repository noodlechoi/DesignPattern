...
17. [데이터 지역성](#chapter-17-데이터-지역성)
18. [더티 플래그](#chapter-18-더티-플래그)
19. [객체 풀](#chapter-19-객체-풀)
20. [공간 분할](#chapter-20-공간-분할)
# Chapter 1. 구조, 성능, 게임
## 1.1 소프트웨어 구조란?
  ### 좋은 소프트웨어 구조란?
  - 구조는 변경과 관련 있다.
...
- 지속 자료구조(persistent data structure): 어떤 객체를 변경하면 원래 객체는 그대로 두고 새로운 객체를 반환한다. 명령 객체마다 명령 실행 전에 객체를 참조하고 있다가, 실행을 취소할 때 객체를 이전 객체로 되돌려주면 된다.
...
- Direct3D, OpenGL 모두 __인스턴스 렌더링__ 을 지원한다.
...
- 멀티스레드, 락(lock)과 함꼐 사용 시에 교착상태에 빠지지 않도록 주의해야 한다. 엔에서 멀티스레드를 많이 사용한다면, 이벤트 큐(15장)을 이용해 비동기적으로 상호작용하는 것이 좋다.
...
# Chapter 17. 데이터 지역성
- 의도: CPU 캐시를 최대한 활용할 수 있도록 데이터를 배치해 메모리 접근 속도를 높인다.
- 무어의 법칙대로 데이터 연산은 빨라졌지만, 데이터를 *가져오는* 건 그다지 빨라지지 않았다. 데이터를 주메모리에서 레지스터로 가져와야 하는데, RAM은 CPU 속도를 따라잡지 못했다.
### CPU가 대기 상태가 아닌 이유
- **CPU 캐싱** 을 사용하기 때문이다.
- CPU 캐시: CPU 안에 있는 작은 메모리로, 주메모리보다 빠르게 데이터를 전달할 수 있다. 정적 램(SRAM)을 사용한다. L1, L2, L3과 같이 여러 계층으로 나누고, 크기가 커질수록 느려진다.
- 캐시 라인(cache line): Ram에서 연속된 메모리를 선택해 캐시에 복사하는 메모리 덩어리
- [캐시 집합(cache associativity)](https://mydailylogs.tistory.com/32)
- 캐시 히트(cache hit): 캐시에서 원하는 데이터를 찾는 것
- 캐시 미스(cache miss): 데이터를 찾지 못해 주메모리에서 데이터를 가져오는 것. 이것이 발생하면 필요한 데이터가 없기 때문에 CPU는 멈춘다.
- __자료구조를 잘 만들어서 처리하려는 값이 메모리 내에서 서로 가까이 붙어 있도록 하는 것__ 이 목표다.
## 패턴
데이터 지역성을 높일 수록 성능을 향상시킬 수 있다.
### 언제 쓸 것인가?
- 성능 문제가 있을 때 사용해야 한다. 성능 문제가 **캐시 미스** 때문인지를 확인해야 한다.
...
- [X] [데이터 지역성 예제 코드](https://github.com/noodlechoi/DesignPattern/tree/main/PracticeGame/Chapter17)
## 디자인 결정
언제 어디에 이 패턴을 사용할 것인지 가장 중요하다. 그 외에 신경 써야하는 것들은 아래와 같다.
### 다형성은 어떻게 할 것인가?
#### 사용하지 않는다
- 안전하고 쉽다. 모든 객체는 정확하게 같은 크기이다.
- 더 빠르다. [동적 디스패치](https://velog.io/@eddy_song/dynamic-dispatch)를 사용하려면 vtable에서 메서드를 찾아본 다음에 포인터를 통해서 실제 코드를 찾아야 한다.
- __유연하지 않다.__
#### 종류별로 다른 배열에 넣는다
- 객체를 빈틈없이 담을 수 있다.
- 정적 디스패치를 할 수 있다.
- 여러 컬렉션을 관리해야 한다.
- 모든 자료형을 알고 있어야 한다. 전체 클래스 자료형과 커플링된다.
#### 하나의 컬렉션에 포인터를 모아놓는다
배열 하나에 상위 클래스나 인터페이스의 포인터를 저장한다.
- 유연하다.
- 캐시 친화적이지 않다. 코드가 딱히 성능에 민감하지 않다면 큰 문제가 없다.
### 게임 개체는 어떻게 정의할 것인가?
#### 게임 개체 클래스가 자기 컴포넌트를 포인터로 들고 있을 때
처음 본 예제가 이런 식이다. 실제 컴포넌트가 메모리 어디에, 어떻게 들어 있는지 전혀 알 수 없다.
- 컴포넌트들을 연속된 배열에 저장할 수 있다. 컴포넌트들을 정렬된 배열에 둬서 순회 작업을 최적화할 수 있다.
- 개체로부터 개체 컴포넌트를 쉽게 얻을 수 있다.
- __컴포넌트를 메모리에서 옮기기가 어렵다.__
#### 게임 개체 클래스가 컴포넌트를 ID로 들고 있을 때
ID를 컴포넌트의 추상 식별자로 사용해 실제 컴포는트 객체 레퍼런스를 얻는다. 컴포넌트 별로 ID를 발급한 뒤에 배열에서 찾거나 해시 테이블로 관리해도 된다.
- 더 복잡하다. 시스템을 구현하고 디버깅해야 하고, 메모리도 더 필요하다.
- 더 느리다.
- 컴포넌트 *관리자* 같은 것에 접근해야 한다. 컴포넌트 등록소(registry)도 있어야 한다.
#### 게임 개체가 단순히 ID일 때
**컴포넌트가 자기 개체를 알게 만든다.** 모든 컴포넌트는 자신을 소유하는 개체의 ID를 기억한다. 자기와 같은 개체 ID를 가진 컴포넌트를 찾으면 형제 컴포넌트에 접근할 수 있다. 개체 클래스는 완전히 사라진다.
- 개체가 단순해진다.
- 개체가 비어 있다. 이 방식은 *컴포넌트 패턴(14장)* 에 더 집중하게 된다.
- 개체 생명주기를 관리하지 않아도 된다. 따로 할당하거나 해제하지 않아도 된다.
- __특정 개체의 컴포넌트를 찾는 게 느릴 수 있다.__ *해결법* 은 컴포넌트를 종류별로 들고 있는 배열에서의 인덱스를 개체 ID로 삼는 것이다. 하지만 모든 배열을 병렬로 유지해야 한다(p343 참고).
## 관련 자료
- 리처드 파비앙의 Data-oriented Design
- 노엘 로피스(Noel Llopis)의 [Data-oriented Design](https://gamesfromwithin.com/data-oriented-design), [번역](https://goo.gl/3trNir)
- 책 프로그래머가 몰랐던 멀티코어 CPU 이야기
- Cachegrid: 캐시 상호작용 결과를 알려줌
- 토니 알브레히트의 pitfalls of object oriented programming

# Chapter 18. 더티 플래그
의도: 불필요한 작업을 피하기 위해 실제로 필요할 때까지 그 일을 미룬다.   
많은 게임에서 월드에 들어 있는 모든 객체를 **[장면 그래프(scene graph)](https://go-hard.tistory.com/8)** 라는 큰 자료구조에 저장한다. 장면 그래프는 거의 *계층형* 이다. 절대 위치가 아닌 상위 위치에 상대적인 값으로 저장된다. 객체를 그리기 위해서는 지역 변환 값(local transform)이 나닌 **월드 변환 값(world transform)** 을 알아야 한다.   
상위 객체가 이동하면 자신의 월드 변환뿐만 아니라 전체 하위 객체에게도 *재귀적으로* 영향을 미치기 때문에 객체 행렬 값을 최신으로 유지하기가 쉽지 않다. *해결법* 은 변환 값을 **캐시** 하는 것이다. 모든 객체의 지역 변환 값과 파생 월드 변환 값을 저장한다. 객체가 움직이면 월드 변환 값도 같이 업데이트한다. 그러나 상위 객체가 움직이면 하위 객체들의 월드 변환 값도 재귀적으로 전부 재계산해야 한다.
### 재계산 미루기(해결법)
지역 변환 값 변경과 월드 변환 값 업데이트를 분리한다. 필요한 지역 변환 값부터 한 번에 변경한 뒤에 업데이트 해야 하는 월드 변환 값을 __렌더링 직전에 한 번만 재계산한다.__ 장면 그래프에 들어가는 객체에 **플래그(flag)** 를 추가한다.   
지역 변환 값이 바뀌면 플래그를 켠다. 객체의 월드 변환 값이 필요할 때에는 플래그를 검사한다. 플래그가 월드 변환을 계산한 뒤에 플래그를 끈다. 이 패턴에서는 1비트만으로 다음과 같은 결과를 얻을 수 있다.
- 객체당 한 번의 재계산으로 합친다.
- 움직이지 않는 객체는 변환 계산을 하지 않는다.
- 렌더링 전에 제거될 객체는 월드 변환 계산을 하지 않아도 된다.
## 패턴
더티 플래그는 **기본 값이 변경되면 켜진다.** 플래그가 켜져 있다면 재계산 후 플래그를 끄고, 꺼져 있다면 이전에 캐시해놓은 파생 값을 그대로 사용한다.
### 언제 쓸 것인가?
코드가 복잡해지는 것을 감수할 정도로 **성능 문제가 심할 때에만 사용해야 한다.** 이 패턴은 *계산* 과 *동기화* 작업에 사용한다. 두 작업은 기본 값으로부터 파생 값을 얻는게 오래 걸리거나 비용이 크다는 문제가 있다. 다른 *요구사항* 들은 다음과 같다.
- 파생 값이 사용되는 횟수보다 기본 값이 더 자주 변경되어야 한다.
- 점진적으로 업데이트하기가 어려워야 한다(p350 참고).
### 주의사항
#### 너무 오래 지연하려면 비용이 든다
전체를 처리하는 데 상당한 시간이 걸리는 작업을 결과를 보고 싶어할 때 한다면 *화면 멈춤* 현상이 생길 수 있다(p351 참고). 또한 작업 지연 시 작업이 전부 날아갈 수도 있다. 영속적인 상태 저장에 더티 플러그 패턴을 사용할 때 더 그렇다.
...
- [X] [더티 플래그 예제 코드](https://github.com/noodlechoi/DesignPattern/tree/main/PracticeGame/Chapter18)
## 디자인 결정
### 더티 플래그를 언제 끌 것인가?
#### 결과가 필요할 때
- 결과 값이 필요 없다면 아예 계산하지 않을 수 있다. 기본 값이 파생 값보다 훨씬 자주 바뀔 때 좋다.
- 계산 시간이 오래 걸린다면 거슬리는 멈춤 현상이 생길 수 있다.
#### 미리 정해놓은 지점에서 할 때
해적선을 항구에 정박할 때만 저장하거나 게임 시스템과 상관없이 로딩화면이나 컷신이 나오는 동안 작업을 처리할 수도 있다.
- 지연 작업 처리가 플레이 경험에 영향을 미치지 않는다.
- 전과는 반대로 작업 처리 시점을 제어할 수 없다.
#### 백그라운드로 처리할 때
처음 값을 변경할 때 정해진 *타이머* 를 추가하고 타이머가 돌아왔을 때 지금까지의 변경사항을 처리한다. [이력현상(hysteresis)](https://goo.gl/Jg0EMy)
- 얼마나 자주 작업을 처리할지 조절할 수 있다.
- 필요 없는 작업을 더 많이 할 수 있다.
- 비동기 작업을 지원해야 한다. 동시적으로 데이터를 안전하게 변경할 수 있도록 해야 한다.
### 더디 플래그는 값 변화를 얼마나 세밀하게 관리해야 하는가?
서버에 보내는 데이터에는 변경된 객체에 대한 데이터와 어디가 변경되었는지를 나타내는 메타데이터가 들어있다.
#### 더 세밀하게 관리된다면
 모든 객체에 더티 플래그를 붙인다고 가정한다.
- 실제로 변경된 데이터만 처리한다. 정확하게 변경된 부분만 서버로 전달한다.
#### 더 듬성듬성하게 관리한다면
- 변경 안 된 데이터도 같이 처리해야 한다.
- 더티 플래그에 드는 메모리가 줄어든다.
- 고정 오버헤드에 드는 시간을 줄일 수 있다. 메타데이터가 고정 작업에 해당한다.
## 관련자료
- 이 패턴은 앵귤러(Argular) 같은 브라우저-사이드 웹 프레임워크에서도 흔히 사용된다.
- 물리 엔진에서는 어떤 객체가 움직이는 중인지 멈춰 있는지 확인한다.

# Chapter 19. 객체 풀
- 의도: 객체를 매번 할당, 해제하지 않고 고정 크기 풀에 들어 있는 객체를 재사용함으로써 메모리 사용 성능을 개선한다.   
단편화와 메모리 할당 속도 때문에 생기는 문제를 해결해준다.
## 패턴
재사용 가능한 객체들을 모아놓은 **객체 풀 클래스** 를 정의한다. 여기에 들어가는 객체는 현재 자신이 *사용 중* 인지 여부를 알 수 있는 방법을 제공해야 한다. 풀이 초기화될 때 사용할 객체를 미리 생성하고, 이들 객체를 *사용 안함* 상태로 초기화한다. 새로운 객체가 필요하면 풀에 요청한다. 풀은 사용 가능한 객체를 찾아 사용 중으로 초기화한 뒤 반환한다.
### 언제 쓸 것인가?
이 패턴은 게임 개체나 시각적 효과, 출력 중인 사운드 같은 객체에 많이 사용된다. 다음과 같을 때 사용하면 좋다.
- 객체를 빈번하게 생성/삭제해야 한다.
- 객체들의 크기가 비슷하다.
- 객체를 힙에 생성하기가 느리거나 메모리 단편화가 우려된다.
- 데이터베이스 연결이나 네트워크 연결같이 접근 비용이 비싸면서 재사용 가능한 자원을 객체가 캡슐화하고 있다.
### 주의사항
원도우에서 16KB 이하 메모리를 많이 할당한다면 [LFH(low-fragmentation heap)](https://learn.microsoft.com/ko-kr/windows/win32/memory/low-fragmentation-heap?redirectedfrom=MSDN)를 쓸 때 더 효과가 좋다.
#### 객체 풀에서 사용되지 않는 객체는 메모리 낭비와 다를 바 없다
크기가 작으면 크래시가 날 테니 바로 알 수 있다. 하지만 동시에 풀이 너무 *커지지 않도록* 주의해야 한다.
#### 한 번에 사용 가능한 객체 개수가 정해져 있다
객체 풀의 모든 객체가 사용 중이어서 재사용할 객체를 반환받지 못할 때를 대비해야 한다. 대비책은 아래와 같다.
- 이런 일이 아예 생기지 않게 한다. 사용자가 어떻게 하더라도 풀이 절대 부족하지 않도록 **풀의 크기를 조절한다.** 그러나 몇번 없을 최악의 상황에 맞춰서 객체 풀의 크기를 유지해야 한다. 상황에 따라 풀의 크기를 다르게 조절할 수 있는지 고려해야 한다.
- 객체를 **생성하지 않는다.** 파티클 시스템 같은 곳에서 가능하다. 이미 모든 파티클이 사용 중일 때 생성하지 않아도 눈에 띄지 않는다.
- 기존 객체를 강제로 **제거** 한다. 기존 객체가 사라지는 것이 새로운 객체가 안 보이는 것보다 눈에 덜 뛸 때 이 방법을 사용한다.
- 풀의 크기를 **늘린다.** 런타임에 풀의 크기를 늘리거나 보조 풀을 만들 수도 있다. 더 이상 쓰지 않을 때 크기를 줄일 것인지 그대로 둘 것인지 정해야 한다.
#### 객체를 위한 메모리 크기는 고정되어 있다
대부분 객체 풀에서 객체를 배열로 관리하도록 구현한다. 다른 자료형이나 추가된 하위 클래스의 인스턴스를 같은 풀에 넣고 싶다면, 풀의 배열 한 칸 크기를 **가장 큰** 자료형에 맞춰야 한다. 이때 __다음 칸에 들어 있는 객체의 메모리를 덮어써버린다.__ 또한 객체 크기가 다양하면 메모리를 낭비하게 된다. __해결법은 객체 크기별로 풀을 나누는 것이다.__
...
- [X] [객체 풀 예제 코드](https://github.com/noodlechoi/DesignPattern/tree/main/PracticeGame/Chapter19)
## 디자인 결정
게임에 객체 풀을 구현하려면 다음과 같은 사항에 대해서 결정해야 한다.
### 풀이 객체와 커플링되는가?
**객체가 자신이 풀에 들어 있는지** 를 알게 할 것인지부터 결정해야 한다.
#### 커플링된다면
- 더 간단하게 구현할 수 있다. 풀에 들어가는 객체에 *사용 중* 플래그나  함수를 추가하면 된다.
- 객체가 풀을 통해서만 생성할 수 있도록 강제할 수 있다. 풀 클래스를 객체 클래스의 friend로 만든 뒤 객체 생성자를 private에 두면 된다.
```
class Particle
{
	friend class ParticlePool;
private:
	Particle() : isUse_(false) {}
	bool isUse_;
};
...
```
이런 관계는 객체 클래스를 어떻게 사용해야 하는지 *문서화* 하는 효과도 있다.
- 사용 중 플래그가 꼭 필요한 건 아닐 수 있다. 객체에 자신이 사용 중인지 알 수 있는 상태가 이미 있는 경우가 많다.
#### 커플링되지 않는다면
- 어떤 객체라도 풀에 넣을 수 있다.
- 사용 중 상태를 객체 외부에서 관리해야 한다. 가장 간단한 방법은 *비트 필드* 를 따로 두는 것이다.
```
...
private:
	...
	bool isUse_[POOL_SIZE];
};
```
### 재사용되는 객체를 초기화할 때 어떤 점을 주의해야 하는가?
#### 객체를 풀 안에서 초기화한다면
- 풀은 객체를 완전히 캡슐화할 수 있다. 캡슐화는 하되 객체를 참조할 수 있게 하려면, 객체 포인터가 아닌 *핸들 값* 을 반환하는 방법도 있다.
- 풀 클래스는 객체가 초기화되는 방법과 결합된다. 풀에서 초기화 관리를 하려면, 풀에 있는 객체의 **모든 초기화 메서드를 풀에서 지원** 해야 하고 객체도 포워딩해야 한다.
#### 객체를 밖에서 초기화한다면
- 풀의 인터페이스는 단순해진다. 초기화 함수를 전부 제공할 필요없이 새로운 객체에 대한 *레퍼런스만 반환* 하면 된다.
- 외부 코드에서는 객체 생성이 실패할 떄의 처리를 챙겨야 할 수 있다. 안전을 위해 객체를 초기화하기 전에 검사하는 게 좋다.
```
Particle* particle = pool.create();
if (particle != nullptr) particle->init(1, 2);
```
## 관련자료
객체 풀 패턴은 경량 패턴(3장)과 비슷한 점이 많다. 둘 다 **재사용** 가능한 객체 집합을 관리한다.
- 경량 패턴: 같은 인스턴스를 여러 객체에서 *공유* 함으로써 재사용한다. 메모리 **중복** 사용을 피하려는 의도다.
- 객체 풀 패턴: 여러 곳에서 동시에 재사용하지 않는다. 이전 사용자가 다 쓴 다음에 객체 메모리를 *회수* 해 간다.

# Chapter 20. 공간 분할
의도: 객체를 효과적으로 찾기 위해 **객체 위치** 에 따라 구성되는 자료구조에 저장한다.   
공간(space) 개념을 구현하기 위해서 **주변에 어떤 객체들이 있는지** 알아야 한다. 이것을 매 프레임마다 확인한다면 성능 병목이 될 수 있다. 이때 위치에 따라 구성되는 자료구조에 객체를 저장하면 객체를 훨씬 빠르게 찾을 수 있다. 이것을 2차원 이상 공간에 적용한 것이 공간 분할 패턴이다.
## 패턴
객체들은 공간 위에서 위치 값을 갖는다. 이들 객체 위치에 따라 구성되는 **공간 자료구조** 에 저장한다. 공간 자료구조를 통해서 같은 위치나 주변에 있는 객체를 빠르게 찾을 수 있다. 객체 위치가 바뀌면 공간 자료구조도 *업데이트* 한다.
### 언제 쓸 것인가?
- 살아 움직이는 게임 객체뿐만 아니라 정적인 프랍이나 지형 저장에 흔히 사용된다.
...
- [X] [공간 분할 예제 코드](https://github.com/noodlechoi/DesignPattern/tree/main/PracticeGame/Chapter20)
## 디자인 결정
각 자료구조에 대한 설명이다.
### 공간을 계층적으로 나눌 것인가, 균등하게 나눌 것인가?
 앞서 본 예제에서는 공간을 균등한 칸으로 나눴다. 반대로 **계층적 공간 분할** 에서는 먼저 공간을 몇 개의 영역으로 나눈다. 그런 뒤에 객체가 많은 영역은 다시 분할한다. 모든 영역에 있는 유닛 개수가 특정 최대 개수 이하로 떨어질 때까지 이 과정을 *재귀적으로* 반복한다.
#### 균등하게
- 더 단순하다.
- 메모리 사용량이 일정하다.
...
**적응형 분할 방식** 에서는 유닛 개수와 위치에 따라 분할 크기를 조절할 수 있다. 목표는 성능을 높이기 위해 영역마다 비슷한 수의 유닛이 들어 갈 수 있도록 *균형 잡힌* 분할을 만드는 것이다.   
격자 예제에서는 격자 한 칸에 유닛이 몰려 있으면, 공격 대상을 찾는 코드 성능이 O(n^2)로 퇴보한다.
#### 객체 개수와 상관없이
- 객체는 순차적으로(incrementally) 추가될 수 있다. 성능 걱정없이 한 번에 처리할 수 있다.
- 객체가 빠르게 이동할 수 있다. 객체에 개수에 따라 분할한다면, 유닛 하나만 이동해도 다른 많은 유닛까지 영역을 옮겨야 할 수 있다.
- 영역이 균형 잡혀 있지 않을 수 있다.
#### 객체 개수에 따라
*이진 공간 분할(BSP)* 나 *k-d 트리* 같은 공간 분할에서는 양쪽에 같은 수의 객체가 들어 있도록 월드 공간을 반씩 *재귀적으로* 쪼갠다. 이를 위해서 공간을 분할할 평면을 선택했을 때, 양쪽에 객체가 각각 몇 개씩인지 계산해야 한다. *경계 볼륨 계층구조(BVH)* 는 객체 분포에 맞춰서 공간을 분할하는 또 다른 기법이다.
- 영역의 균형 잡힙(balanced)을 보장할 수 있다. 성능을 *일정하게* 유지할 수 있다.
- 전체 객체에 대해 한 번에 분할해놓는 게 훨씬 효과적이다. 전체 객체를 *미리* 준비해둬야 한다. 보통 고정되어 있거나 정적 지형이나 아트 리소스에 자주 사용된다.
#### 영역 분할은 고정되어 있지만, 계층은 객체 개수에 따라 달라진다면
*쿼드트리(quadtree)* 는 고정 분할과 적응형 분할의 장점을 어느 정도 가지는 공간 분할 방식이다. 3차원 공간에서는 8개의 정육면체로 분할하는 옥트리(octree)가 있다. 쿼드트리는 전체 공간을 한 영역으로 시작한다. 공간에 들어 있는 객체 개수가 정해진 수 이상 넘어가면 사각형 4개로 분할되는데, 이때 크기는 1/4로 정해져 있다.   
이렇게 분할된 사각형에 대해서 각 사각형에 들어 있는 객체 개수가 일정 수 이하가 될 때까지 같은 작업을 재귀적으로 반복한다. **분할 영역이 이동하지는 않는다(p392 그림 참고).**
- 객체를 순차적으로 추가할 수 있다.
- 객체 제거가 간단하다.
- 객체 이동이 빠르다.
- 분할 영역이 균형이 잡혀 있다.
### 객체를 공간 분할 자료구조에만 저장하는가?
공간 분할 자료구조에서 게임 객체의 생명주기까지 관리할 수도 있다. 아니면 캐시 용도로만 사용할 수도 있다.
#### 객체를 공간 분할 자료구조에만 저장한다면
- 컬렉션이 두 개가 되면서 생기는 메모리 비용과 복잡도를 피할 수 있다.
#### 다른 컬렉션에도 객체를 둔다면
- 전체 객체를 더 빠르게 순회할 수 있다.
## 관련자료
- 가장 많이 쓰이는 공간 분할 자료구조
    - 격자
    - 쿼드트리
    - 이진 공간 분할(BSP)
    - k-d 트리
